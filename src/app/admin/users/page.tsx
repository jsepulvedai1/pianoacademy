...
import { GET_ADMIN_ACCOUNTS, GET_AUDIT_LOGS, GET_STUDENT_PORTAL_ACCOUNTS, GET_TEACHER_PORTAL_ACCOUNTS } from "@/graphql/queries/admin-queries";
import { CREATE_ADMIN_ACCOUNT, UPDATE_ADMIN_ACCOUNT, DELETE_ADMIN_ACCOUNT, CREATE_STUDENT_ACCOUNT, CREATE_TEACHER_ACCOUNT, RESET_PORTAL_PASSWORD } from "@/graphql/mutations/student-mutations";
...
export default function UsersAdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<'STAFF' | 'STUDENTS' | 'TEACHERS'>('STAFF');
  const [selectedPortalItem, setSelectedPortalItem] = useState<any>(null);
  ...
  const [createStudentAcc, { loading: isCreatingStudent }] = useMutation(CREATE_STUDENT_ACCOUNT, {
    onCompleted: (res: any) => {
      if (res.createStudentAccount?.success) {
        ...
        toast.error(res.createStudentAccount?.error || "Error al crear la cuenta");
      }
    },
  ...
  const [createTeacherAcc, { loading: isCreatingTeacher }] = useMutation(CREATE_TEACHER_ACCOUNT, {
    onCompleted: (res: any) => {
      if (res.createTeacherAccount?.success) {
        ...
        toast.error(res.createTeacherAccount?.error || "Error al crear la cuenta");
      }
    },
    onError: (err: any) => toast.error(err.message)
  });
  ...
  const [resetPortalPass, { loading: isResettingPortal }] = useMutation(RESET_PORTAL_PASSWORD, {
    onCompleted: (res: any) => {
      if (res.resetPortalPassword?.success) {
        ...
        toast.error(res.resetPortalPassword?.error || "Error al restablecer la contraseña");
      }
    },
    onError: (err: any) => toast.error(err.message)
  });
  ...
  const filteredStudents = useMemo(() => {
    return studentsList.filter((s: any) =>
      s.name.toLowerCase().includes(portalSearch.toLowerCase()) ||
      (s.email && s.email.toLowerCase().includes(portalSearch.toLowerCase())) ||
      (s.rut && s.rut.includes(portalSearch))
    );
  }, [studentsList, portalSearch]);

  const filteredTeachers = useMemo(() => {
    return teachersList.filter((t: any) =>
      t.name.toLowerCase().includes(portalSearch.toLowerCase()) ||
      (t.email && t.email.toLowerCase().includes(portalSearch.toLowerCase())) ||
      (t.rut && t.rut.includes(portalSearch))
    );
  }, [teachersList, portalSearch]);
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 py-10 px-4 md:px-8 text-slate-800">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {/* Tab Selector */}
        <div className="bg-slate-200/60 p-1.5 rounded-2xl flex items-center gap-1 w-fit">
          <button 
            onClick={() => { setActiveTab("STAFF"); setPortalSearch(""); }}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all cursor-pointer ${activeTab === "STAFF" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
          >
            <Shield className="h-4 w-4" /> Colaboradores e Historial
          </button>
          <button 
            onClick={() => { setActiveTab("STUDENTS"); setPortalSearch(""); }}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all cursor-pointer ${activeTab === "STUDENTS" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
          >
            <Users className="h-4 w-4" /> Cuentas de Alumnos
          </button>
          <button 
            onClick={() => { setActiveTab("TEACHERS"); setPortalSearch(""); }}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all cursor-pointer ${activeTab === "TEACHERS" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
          >
            <ShieldCheck className="h-4 w-4" /> Cuentas de Profesores
          </button>
        </div>
        ...
        {activeTab === "STAFF" ? (
                ...
                <div className="space-y-4 max-h-[600px] overflow-y-auto pr-1">
                  {accounts.map((user: any) => {
                    ...
                    return (
                      <div 
                        key={user.id} 
                        className="p-4 bg-slate-50 hover:bg-slate-100/50 rounded-2xl border border-slate-100 flex items-center justify-between gap-4 transition-all group"
                      >
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <span className="font-extrabold text-slate-800 text-sm">{user.username}</span>
                            <Badge className={`border font-extrabold text-[8px] uppercase tracking-widest scale-90 ${roleCfg.bg} ${roleCfg.color}`}>
                              {roleCfg.label}
                            </Badge>
                          </div>
          ...
          <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100 space-y-6 animate-in fade-in duration-200">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
              <h2 className="text-base font-extrabold tracking-widest text-slate-400 uppercase flex items-center gap-2">
                {activeTab === "STUDENTS" ? <Users className="h-4 w-4 text-[#70125F]" /> : <ShieldCheck className="h-4 w-4 text-[#70125F]" />} 
                {activeTab === "STUDENTS" ? "Gestión de Portales Alumnos" : "Gestión de Portales Profesores"}
              </h2>
              <div className="relative w-full sm:w-80">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-slate-400" />
                <Input 
                  placeholder={activeTab === "STUDENTS" ? "Buscar alumno por nombre, RUT o email..." : "Buscar profesor por nombre, RUT o email..."} 
                  value={portalSearch} 
                  onChange={(e) => setPortalSearch(e.target.value)}
                  className="pl-9 h-11 bg-slate-50 border-none rounded-xl text-xs" 
                />
              </div>
            </div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-slate-100 text-slate-400 font-bold uppercase tracking-wider text-[10px]">
                    ...
                    <th className="py-4 px-3">RUT / Identificación</th>
                    <th className="py-4 px-3">Email</th>
                    {activeTab === "STUDENTS" && <th className="py-4 px-3">Nivel</th>}
                    <th className="py-4 px-3">Estado de Cuenta</th>
                    <th className="py-4 px-3">Username Portal</th>
                    <th className="py-4 px-3 text-right">Acciones</th>
                  </tr>
                </thead>
                <tbody>
                  {(activeTab === "STUDENTS" ? filteredStudents : filteredTeachers).map((item: any) => {
                    const hasAccount = !!item.user;
                    return (
                      <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                        <td className="py-4 px-3 font-bold text-slate-700">{item.name}</td>
                        <td className="py-4 px-3 font-medium text-slate-500">{item.rut || "—"}</td>
                        <td className="py-4 px-3 font-medium text-slate-500">{item.email || "—"}</td>
                        {activeTab === "STUDENTS" && <td className="py-4 px-3"><Badge className="bg-slate-100 text-slate-600 border-none font-bold text-[9px] uppercase tracking-wider">{item.level}</Badge></td>}
                        <td className="py-4 px-3">
                          {hasAccount ? (
                            <Badge className="bg-emerald-50 text-emerald-700 border-none font-extrabold flex items-center gap-1 w-fit">
                              <Check className="h-3 w-3" /> Activa
                            </Badge>
                          ) : (
                            <Badge className="bg-slate-100 text-slate-400 border-none font-extrabold w-fit">
                              Sin Cuenta
                            </Badge>
                          )}
                        </td>
                        <td className="py-4 px-3 font-mono font-bold text-[#70125F]">
                          {hasAccount ? item.user.username : "—"}
                        </td>
                        <td className="py-4 px-3 text-right">
                          {hasAccount ? (
                            <Button 
                              ...
                              className="bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl h-9 px-4 font-bold text-xs uppercase cursor-pointer transition-all flex items-center gap-1.5 ml-auto"
                            >
                              <Key className="h-3.5 w-3.5" /> Cambiar Contraseña
                            </Button>
                          ) : (
                            <Button 
                              onClick={() => {
                              ...
                              className="bg-[#70125F]/10 hover:bg-[#70125F]/20 text-[#70125F] rounded-xl h-9 px-4 font-bold text-xs uppercase cursor-pointer transition-all flex items-center gap-1.5 ml-auto"
                            >
                  ...
                  {(activeTab === "STUDENTS" ? filteredStudents : filteredTeachers).length === 0 && (
                    <tr>
                      <td colSpan={activeTab === "STUDENTS" ? 7 : 6} className="py-8 text-center text-slate-400 italic">No se encontraron registros.</td>
                    </tr>
                  )}
                </tbody>
              </table>
      ...
      {/* MODAL: Create Student/Teacher Portal Account */}
      {isCreatingPortalAccount && selectedPortalItem && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl space-y-6 border border-slate-100">
            <div className="flex items-center justify-between border-b pb-4">
              <h2 className="text-xl font-extrabold text-slate-900 font-serif tracking-tight flex items-center gap-2">
                <ShieldCheck className="h-6 w-6 text-[#70125F]" /> Crear Cuenta de Portal
              </h2>
              <button 
                onClick={() => {
                  setIsCreatingPortalAccount(false);
            ...
            <div className="space-y-4 text-xs text-slate-500 leading-relaxed bg-slate-50 p-4 rounded-2xl border border-slate-100">
              <p>Generando cuenta para: <strong className="text-slate-800">{selectedPortalItem.name}</strong></p>
              <p>
                El nombre de usuario se generará automáticamente a partir del RUT o correo electrónico:
                <br />
                <strong className="text-[#70125F] font-mono text-sm block mt-1">
                  {selectedPortalItem.rut || selectedPortalItem.email || `portal_${selectedPortalItem.id}`}
                </strong>
              </p>
            </div>
            ...
            <form onSubmit={(e) => {
              ...
              if (activeTab === "STUDENTS") {
                createStudentAcc({ variables: { studentId: parseInt(selectedPortalItem.id), password: portalPassword } });
              } else {
                createTeacherAcc({ variables: { teacherId: parseInt(selectedPortalItem.id), password: portalPassword } });
              }
            }} className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase tracking-widest text-[#70125F]">Contraseña Temporal *</label>
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
                  <Input 
                    type="password"
              ...
              <Button 
                type="submit"
                disabled={isCreatingStudent || isCreatingTeacher}
                className="w-full bg-[#70125F] hover:bg-[#590e4b] text-white rounded-2xl h-12 px-6 font-bold uppercase tracking-wider text-xs shadow-lg shadow-[#70125F]/20 cursor-pointer transition-all"
              >
                {isCreatingStudent || isCreatingTeacher ? "Creando..." : "Confirmar y Crear Cuenta"}
              </Button>
            </form>
          </div>
        </div>
      ...
      {isResettingPortalPassword && selectedPortalItem && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl space-y-6 border border-slate-100">
            <div className="flex items-center justify-between border-b pb-4">
              <h2 className="text-xl font-extrabold text-slate-900 font-serif tracking-tight flex items-center gap-2">
                <Key className="h-6 w-6 text-[#70125F]" /> Restablecer Contraseña
              </h2>
              <button 
                onClick={() => {
                  setIsResettingPortalPassword(false);
            ...
            <div className="text-xs text-slate-500">
              Establecerás una nueva contraseña para la cuenta del portal de: <strong className="text-slate-800">{selectedPortalItem.name}</strong>
            </div>

            <form onSubmit={(e) => {
              e.preventDefault();
              if (!portalPassword.trim()) return;
              resetPortalPass({ variables: { userId: parseInt(selectedPortalItem.user.id), newPassword: portalPassword } });
            }} className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase tracking-widest text-[#70125F]">Nueva Contraseña *</label>
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
                  <Input 
                    type="password"
              ...
              <Button 
                ...
                className="w-full bg-[#70125F] hover:bg-[#590e4b] text-white rounded-2xl h-12 px-6 font-bold uppercase tracking-wider text-xs shadow-lg shadow-[#70125F]/20 cursor-pointer transition-all"
              >
                {isResettingPortal ? "Actualizando..." : "Restablecer Contraseña"}
              </Button>
            </form>
          </div>
        </div>
