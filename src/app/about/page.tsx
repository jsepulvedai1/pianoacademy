...
export default function AboutPage() {
  ...
  const heroTitleHighlight1 = about?.heroTitleHighlight1 || "Aprender";
  const heroTitleText1 = about?.heroTitleText1 || "música debe ser una experiencia";
  const heroTitleHighlight2 = about?.heroTitleHighlight2 || "clara y motivadora.";

  const historyImage = about?.historyImage || "/imagesfooter/2.png";
  const historyTitle = about?.historyTitle || "Nuestra Historia";
  const historySubtitle = about?.historySubtitle || "Academia Detaché nació para transformar la frustración de aprender música en un camino claro y disfrutable.";
  const historyDescription = about?.historyDescription || "Cansados de la improvisación, creamos en el sector sur de Santiago un método estructurado, cercano y apasionado que garantiza un avance real, poniendo siempre el progreso y el bienestar del alumno en el centro de nuestra historia.";

  const movingTitle = about?.movingTitle || "Lo que nos Mueve";
  const movingDescription = about?.movingDescription || "Nuestra forma de enseñar nace del amor por la música y del compromiso con cada estudiante.";
  ...
  if (about?.movingCards) {
    try {
      movingCards = typeof about.movingCards === 'string' ? JSON.parse(about.movingCards) : about.movingCards;
    } catch (e) {
      console.error(e);
    }
  }
  ...
  const finalTitle3 = about?.finalTitle3 || "de aprender";
  const finalImage = about?.finalImage || "/nosotros/4.png";
        ...
        <div className="container relative z-10 px-4 md:px-6 mx-auto">
          <div className="max-w-5xl text-left space-y-6">
            <h1 className="text-5xl md:text-8xl font-extrabold tracking-tight text-white leading-[1.1] font-sans">
              <span className="text-[#DFB012]">{heroTitleHighlight1}</span> {heroTitleText1} <br className="hidden md:block" />
              <span className="text-[#DFB012]">{heroTitleHighlight2}</span>
            </h1>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-white text-slate-800 relative overflow-hidden">
        <div className="container px-4 md:px-6 mx-auto text-center max-w-6xl space-y-16 relative z-10">
          <div className="space-y-4">
            <h2 className="text-4xl md:text-5xl font-extrabold tracking-tight text-[#70125F]">
              {movingTitle}
            </h2>
            <p className="text-slate-800 text-base md:text-lg font-medium max-w-2xl mx-auto">
              {movingDescription}
            </p>
          </div>

          {/* Cards Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 place-items-center sm:place-items-stretch">

            {/* Card 1 */}
            <div 
              className={`relative w-full max-w-[320px] sm:max-w-none rounded-[2rem] md:rounded-[2.5rem] aspect-[4/5] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.15)] transition-all duration-500 cursor-pointer ${activeCard === 1 ? 'bg-[#EBA9D0] -translate-y-1 shadow-xl' : 'bg-[#70125F]'}`}
              onClick={() => setActiveCard(activeCard === 1 ? null : 1)}
              onMouseEnter={() => setActiveCard(1)}
              onMouseLeave={() => setActiveCard(null)}
            >
              {/* Default State */}
              <div className={`absolute inset-0 p-6 md:p-8 flex flex-col justify-between transition-opacity duration-500 ${activeCard === 1 ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                <h3 className="text-[22px] md:text-2xl font-bold tracking-tight text-left leading-tight text-[#FEF0D4]" dangerouslySetInnerHTML={{ __html: movingCards[0]?.title || "" }} />
                <div className="flex justify-end mt-2 md:mt-4">
                  <svg className="w-36 h-36 md:w-24 md:h-24 text-[#FEF0D4] fill-current" viewBox="0 0 291 355">
              ...
              {/* Hover State */}
              <div className={`absolute inset-0 p-8 flex items-center justify-center transition-opacity duration-500 ${activeCard === 1 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                <p className="text-[#70125F] text-center font-medium md:text-lg leading-relaxed">
                  {movingCards[0]?.description}
                </p>
              </div>
            </div>
            ...
            {/* Card 2 */}
            <div 
              className={`relative w-full max-w-[320px] sm:max-w-none rounded-[2rem] md:rounded-[2.5rem] aspect-[4/5] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.15)] transition-all duration-500 cursor-pointer ${activeCard === 2 ? 'bg-[#EBA9D0] -translate-y-1 shadow-xl' : 'bg-[#70125F]'}`}
              onClick={() => setActiveCard(activeCard === 2 ? null : 2)}
              onMouseEnter={() => setActiveCard(2)}
              onMouseLeave={() => setActiveCard(null)}
            >
              {/* Default State */}
              <div className={`absolute inset-0 p-6 md:p-8 flex flex-col justify-between transition-opacity duration-500 ${activeCard === 2 ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                <h3 className="text-[22px] md:text-2xl font-bold tracking-tight text-left leading-tight text-[#FEF0D4]" dangerouslySetInnerHTML={{ __html: movingCards[1]?.title || "" }} />
                <div className="flex justify-end mt-2 md:mt-4">
                  <svg className="w-32 h-32 md:w-24 md:h-24 text-[#FEF0D4] fill-current" viewBox="0 0 204 292">
              ...
              {/* Hover State */}
              <div className={`absolute inset-0 p-8 flex items-center justify-center transition-opacity duration-500 ${activeCard === 2 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                <p className="text-[#70125F] text-center font-medium md:text-lg leading-relaxed">
                  {movingCards[1]?.description}
                </p>
              </div>
            </div>
            ...
            {/* Card 3 */}
            <div 
              className={`relative w-full max-w-[320px] sm:max-w-none rounded-[2rem] md:rounded-[2.5rem] aspect-[4/5] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.15)] transition-all duration-500 cursor-pointer ${activeCard === 3 ? 'bg-[#EBA9D0] -translate-y-1 shadow-xl' : 'bg-[#70125F]'}`}
              onClick={() => setActiveCard(activeCard === 3 ? null : 3)}
              onMouseEnter={() => setActiveCard(3)}
              onMouseLeave={() => setActiveCard(null)}
            >
              {/* Default State */}
              <div className={`absolute inset-0 p-6 md:p-8 flex flex-col justify-between transition-opacity duration-500 ${activeCard === 3 ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                <h3 className="text-[22px] md:text-2xl font-bold tracking-tight text-left leading-tight text-[#FEF0D4]" dangerouslySetInnerHTML={{ __html: movingCards[2]?.title || "" }} />
                <div className="flex justify-end mt-2 md:mt-4">
                  <svg className="w-36 h-36 md:w-24 md:h-24 text-[#FEF0D4] fill-current" viewBox="0 0 255 229">
              ...
              {/* Hover State */}
              <div className={`absolute inset-0 p-8 flex items-center justify-center transition-opacity duration-500 ${activeCard === 3 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                <p className="text-[#70125F] text-center font-medium md:text-lg leading-relaxed">
                  {movingCards[2]?.description}
                </p>
              </div>
            </div>
            ...
            {/* Card 4 */}
            <div 
              className={`relative w-full max-w-[320px] sm:max-w-none rounded-[2rem] md:rounded-[2.5rem] aspect-[4/5] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.15)] transition-all duration-500 cursor-pointer ${activeCard === 4 ? 'bg-[#EBA9D0] -translate-y-1 shadow-xl' : 'bg-[#70125F]'}`}
              onClick={() => setActiveCard(activeCard === 4 ? null : 4)}
              onMouseEnter={() => setActiveCard(4)}
              onMouseLeave={() => setActiveCard(null)}
            >
              {/* Default State */}
              <div className={`absolute inset-0 p-6 md:p-8 flex flex-col justify-between transition-opacity duration-500 ${activeCard === 4 ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                <h3 className="text-[22px] md:text-2xl font-bold tracking-tight text-left leading-tight text-[#FEF0D4]" dangerouslySetInnerHTML={{ __html: movingCards[3]?.title || "" }} />
                <div className="flex justify-end mt-2 md:mt-4">
                  <svg className="w-36 h-36 md:w-24 md:h-24 text-[#FEF0D4] fill-current" viewBox="0 0 260 218">
              ...
              {/* Hover State */}
              <div className={`absolute inset-0 p-8 flex items-center justify-center transition-opacity duration-500 ${activeCard === 4 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                <p className="text-[#70125F] text-center font-medium md:text-lg leading-relaxed">
                  {movingCards[3]?.description}
                </p>
              </div>
            </div>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            ...
            <div className="space-y-10 text-left">
              <h2 className="text-4xl md:text-6xl font-extrabold tracking-tight leading-[1.1] font-sans uppercase">
                <span className="text-[#DFB012]">{finalTitle1}</span> <br />
                <span className="text-[#70125F]">{finalTitle2}</span> <br />
                <span className="text-[#DFB012]">{finalTitle3}</span>
              </h2>
              <div>
